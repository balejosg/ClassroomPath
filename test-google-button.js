...
async function testGoogleButton() {
  ...
  try {
    ...
    const googleButton = await page.locator(
      '#google-signin-btn iframe, #google-signin-btn div[role="button"]'
    );
    const buttonCount = await googleButton.count();
    console.log(
      `   ${buttonCount > 0 ? '✅' : '❌'} Botón de Google encontrado: ${buttonCount} elementos`
    );
    ...
    const registerButton = await page.locator(
      '#google-signup-btn iframe, #google-signup-btn div[role="button"]'
    );
    const registerButtonCount = await registerButton.count();
    console.log(
      `   ${registerButtonCount > 0 ? '✅' : '❌'} Botón en registro: ${registerButtonCount} elementos`
    );
