...
- Release hardening commands are diagnostic-first. Use
  ...
  `npm run release:promote -- --rc-run-id <RC_RUN_ID> --auto-tag --dry-run` prints
  the ordered RC-first plan without deploying, tagging, waiting for production, or
  running health checks.
- The promotion orchestrator resolves one explicit RC run, deploys that exact
  bundle to staging through the shared executor, verifies exact staging identity,
  runs canonical production readiness (recovery/config/host/artifacts), then
...
| Broken staging deployment                     | `npm run deploy:staging -- --rc-run-id <RC_RUN_ID>`                                                                                                               | developer machine + staging host                  | deploys the explicit RC through the shared executor, runs live verification, and records evidence |
