...
| release-orchestration.test.ts                | 648   | `scripts/lib/release-orchestration.mjs`, `scripts/release-promote.mjs`: high-risk step order, production deploy/health commands, optional local canary diagnostic, dry-run non-mutating, prepromotion refresh                                                                         | Release promotion orchestrator changes                              |
...
| Test file                                      | Lines | Guards                                                                                                                                                                    | Breaks when                                                                      |
| ---------------------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| tenant-group-rules.test.ts                     | 188   | TenantGroupRules: createRule authorization + OpenPath write + publication, bulkCreateRules, updateRule, deleteRule                                                        | Tenant group rule service or OpenPath write contract changes                     |
...
| windows-offline-installer-architecture.test.ts | 91    | Windows installer ownership regression guard: no generic ClassroomPath lifecycle, no active legacy storage wiring, and explicit retired route policy                      | Installer ownership, legacy storage, or retired route policy changes             |
...
**Files in this category: 9**

| Test file                                                          | Lines | Guards                                                                                                                                                                                                                                                                                                                                                             | Breaks when                                                                              |
| ------------------------------------------------------------------ | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| deployment-runtime-config.test.ts [ops-contract]                   | 618   | `docker-compose.yml`, `Dockerfile.api`, `.env.example`: API/gateway/SPA service config, host.docker.internal gateway, platform pins, OpenPath submodule reference, Windows bootstrap assets, canonical installer roots, required env vars, email/Linux-agent-pin documentation                                                                                     | docker-compose.yml, Dockerfile.api, or .env.example edited                               |
| windows-offline-installer-legacy-retirement.test.ts [ops-contract] | 235   | One-shot legacy storage retirement: confirmation gate, exact Compose labels/name/driver, idempotent absence, canonical-volume exclusion, and no normal deploy invocation                                                                                                                                                                                           | Legacy artifact retirement helper or migration sequencing changes                        |
| windows-offline-installer-volume-runtime.test.ts [ops-contract]    | 22    | Fresh named-volume smoke harness contract: image build, explicit volume mounts, non-root probes, read-only template mount, private artifact mount, and scoped cleanup                                                                                                                                                                                              | Dockerfile runtime roots or fresh-volume smoke changes                                   |
...
| production-remote-regressions.test.ts [ops-contract]               | 592   | Real streamed production bootstrap from a previous-release fixture, real recovery entrypoint with candidate helpers broken, stable post-switch diagnostic fallback, and marker-preserving selector matrix                                                                                                                                                          | Production remote bootstrap, recovery bundle, or post-switch diagnostic workflow changes |
| staging-equivalent-harness.test.ts [ops-contract]                  | 1071  | `scripts/staging-equivalent-harness.sh`: isolated-host fence, production Compose topology, K0 live attestation, Node/npm exclusion, exact R bytes, migration gate, watchdog identity, rollback observation, and bounded evidence                                                                                                                                   | Staging-equivalent harness or #162 K contract changes                                    |
