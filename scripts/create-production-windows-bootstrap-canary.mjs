  ...
  for (const host of explicitSeedHosts) {
    const { data: canaryRule } = await postTrpc(
      'groups.createRule',
      {
        ...
        type: 'whitelist',
        value: host,
        comment: WINDOWS_AJAX_REDDIT_EXPLICIT_ALLOWLIST_HOSTS.includes(host)
          ? 'Windows AJAX Reddit staging diagnostic explicit allowlist experiment'
          : 'Production Windows bootstrap canary explicit AJAX seed rule',
      },
      cookieHeader
    );
