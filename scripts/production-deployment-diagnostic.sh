...
production_deployment_diagnostic_run() {
  ...
  local candidate_sha=""
  local recovery_source_sha=""
  local recovery_contract_version=""
  local recovery_artifact_sha256=""
  local recovery_executor_sha256=""
  local phase=""
  local stage=""
  ...
  candidate_sha="$(diagnostic_state_value "$transaction_file" CANDIDATE_SHA || true)"
  recovery_source_sha="$(diagnostic_state_value "$transaction_file" RECOVERY_SOURCE_SHA || true)"
  recovery_contract_version="$(diagnostic_state_value "$transaction_file" RECOVERY_CONTRACT_VERSION || true)"
  recovery_artifact_sha256="$(diagnostic_state_value "$transaction_file" RECOVERY_ARTIFACT_SHA256 || true)"
  recovery_executor_sha256="$(diagnostic_state_value "$transaction_file" RECOVERY_EXECUTOR_SHA256 || true)"
  ...
  }
  {
    ...
    printf '  "candidateSha":"%s",\n' "$(diagnostic_json_escape "$candidate_sha")"
    printf '  "recoverySourceSha":"%s",\n' "$(diagnostic_json_escape "$recovery_source_sha")"
    printf '  "recoveryContractVersion":"%s",\n' "$(diagnostic_json_escape "$recovery_contract_version")"
    printf '  "recoveryArtifactSha256":"%s",\n' "$(diagnostic_json_escape "$recovery_artifact_sha256")"
    printf '  "recoveryExecutorSha256":"%s",\n' "$(diagnostic_json_escape "$recovery_executor_sha256")"
