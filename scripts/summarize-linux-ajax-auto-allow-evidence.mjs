...
/**
 * Reads Linux AJAX auto-allow canary evidence JSON and writes a Markdown summary for the GitHub Actions step summary.
 *
 * Invoked by: GitHub Actions `linux-production-bootstrap-canary.yml` workflow; `runner-diagnostic-execution.test.ts`.
 * Usage: node scripts/summarize-linux-ajax-auto-allow-evidence.mjs [--input <file>] [--output <file>]
 */
...
import {
  buildLinuxAutoAllowArtifactFailureSummary,
  withLinuxAutoAllowDiagnostics,
} from './lib/linux-auto-allow-canary-evidence.mjs';

const DEFAULT_ARTIFACT_PATH = 'production-linux-ajax-auto-allow-canary.json';

function parseArgs(argv) {
  const options = { artifactPath: DEFAULT_ARTIFACT_PATH, summaryPath: '' };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === '--summary') options.summaryPath = next();
    else if (arg === '--help' || arg === '-h') {
      console.log(`Usage: node scripts/summarize-linux-ajax-auto-allow-evidence.mjs [options]
...
function renderMarkdown(summary) {
  ...
  const lines = [
    '## Linux AJAX Auto-Allow Evidence',
    '',
...
async function main() {
  ...
  if (options.summaryPath) await writeFile(options.summaryPath, `${markdown}\n`, 'utf8');
