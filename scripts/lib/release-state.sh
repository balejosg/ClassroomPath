...
verify_current_release_state_matches_expected() {
  ...
  release_state_assert_equal \
    "OpenPath Windows template commit" \
    "$EXPECTED_OPENPATH_WINDOWS_OFFLINE_TEMPLATE_COMMIT" \
    "${OPENPATH_WINDOWS_OFFLINE_TEMPLATE_COMMIT:-}" || return 1
  release_state_assert_equal \
    "OpenPath Windows template release tag" \
    "$EXPECTED_OPENPATH_WINDOWS_OFFLINE_TEMPLATE_RELEASE_TAG" \
    "${OPENPATH_WINDOWS_OFFLINE_TEMPLATE_RELEASE_TAG:-}" || return 1
  release_state_assert_equal \
    "OpenPath Windows template SHA-256" \
    "$EXPECTED_OPENPATH_WINDOWS_OFFLINE_TEMPLATE_SHA256" \
    "${OPENPATH_WINDOWS_OFFLINE_TEMPLATE_SHA256:-}" || return 1
}
