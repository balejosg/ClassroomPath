...
import {
  buildAllowlistedNavigationSkippedEvidence as buildBrowserAllowlistedNavigationSkippedEvidence,
  buildBlockedPageUnblockRequestSkippedEvidence as buildBrowserBlockedPageUnblockRequestSkippedEvidence,
  buildRedditNavigationSkippedEvidence as buildBrowserRedditNavigationSkippedEvidence,
  collectAllowlistedExternalNavigationDiagnostics as collectBrowserAllowlistedExternalNavigationDiagnostics,
  collectRedditRealNavigationDiagnostics as collectBrowserRedditRealNavigationDiagnostics,
  isBlockedPageUnblockRequestSuccessText,
  runBlockedPageUnblockRequestCheck as runBrowserBlockedPageUnblockRequestCheck,
} from './windows-ajax-browser-checks.mjs';
...
export { isBlockedPageUnblockRequestSuccessText };
...
async function collectRedditDiagnostics(phase, pageEvidence = {}, navigationEvidence = null) {
  ...
  return redactWindowsCanaryObject({
    ...
    navigation:
      navigationEvidence ??
      buildBrowserRedditNavigationSkippedEvidence(REDDIT_NAVIGATION_MODE, pageEvidence),
    whitelist: {
      global: globalWhitelist,
      native: nativeWhitelist,
      remoteWhitelist,
  ...
  try {
    ...
    const blockedPageUnblockRequest =
      result?.success && seleniumDriver !== null
        ? await runBrowserBlockedPageUnblockRequestCheck({
            driver: seleniumDriver,
            profileDir,
            firefoxExtensionWarmup,
            config: runtimeConfig,
          })
        : buildBrowserBlockedPageUnblockRequestSkippedEvidence(
            result?.success
    ...
    const redditNavigation =
      result?.success && seleniumDriver !== null
        ? await collectBrowserRedditRealNavigationDiagnostics({
            driver: seleniumDriver,
            mode: REDDIT_NAVIGATION_MODE,
            pageEvidence: redditPageEvidence,
          })
        : buildBrowserRedditNavigationSkippedEvidence(REDDIT_NAVIGATION_MODE, redditPageEvidence);
    ...
    const allowlistedNavigation =
      result?.success && seleniumDriver !== null
        ? await collectBrowserAllowlistedExternalNavigationDiagnostics({ driver: seleniumDriver })
        : buildBrowserAllowlistedNavigationSkippedEvidence(
            result?.success
