...
export const VERIFY_DOMAIN_POLICY_DEFINITIONS = [
  ...
  },
  {
    name: 'deploy-shell',
    patterns: [
      '^scripts/(?:deploy-.+|detect-email-delivery-risk|detect-windows-firefox-risk|package-production-recovery-bundle|production-deployment-diagnostic|production-recovery-authority|rollback-.+|persist-.+|run-migrations(?:-docker|-image)?|run-staging-release-gate|tag-production-release|verify-production-promotion-ready|verify-staging-release-state)\\.sh$',
