...
if [ -n "$SCRIPT_SOURCE" ]; then
  RECOVERY_SOURCE_DIR="$(cd "$(dirname "$SCRIPT_SOURCE")" && pwd)"
  if [ -f "$RECOVERY_SOURCE_DIR/remote-bootstrap.sh" ] &&
    [ -f "$RECOVERY_SOURCE_DIR/remote-deploy-scaffold.sh" ]; then
    SCRIPT_DIR="$(cd "$RECOVERY_SOURCE_DIR/.." && pwd)"
  else
    SCRIPT_DIR="$RECOVERY_SOURCE_DIR"
  fi
else
  printf 'Stable recovery executor requires a transmitted or checked-out source path\n' >&2
  exit 1
fi
...
REMOTE_DEPLOY_SCAFFOLD_HELPER_PATH="$RECOVERY_LIB_DIR/remote-deploy-scaffold.sh"
...
# Resolve every executable dependency inside this bundle. The APP_DIR is data
# and checkout state only; it is never a helper source or fallback root.
COMMON_SH_PATH="$RECOVERY_LIB_DIR/common.sh"
REMOTE_HELPER_CONTRACTS_PATH="$RECOVERY_LIB_DIR/remote-helper-contracts.sh"
RELEASE_STATE_HELPER_PATH="$RECOVERY_LIB_DIR/release-state.sh"
RELEASE_RUNTIME_HELPER_PATH="$RECOVERY_LIB_DIR/release-runtime.sh"
DEPLOYMENT_STATE_HELPER_PATH="$RECOVERY_LIB_DIR/deployment-state.sh"
PRODUCTION_HOST_CONTRACT_HELPER_PATH="$RECOVERY_LIB_DIR/production-host-contract.sh"
DEPLOYMENT_TRANSACTION_HELPER_PATH="$RECOVERY_LIB_DIR/deployment-transaction.sh"
ROLLBACK_EXECUTOR_HELPER_PATH="$RECOVERY_LIB_DIR/rollback-executor.sh"
ROLLBACK_READINESS_HELPER_PATH="$RECOVERY_LIB_DIR/rollback-readiness.sh"
DEPLOY_CONTAINER_PLATFORM_HELPER_PATH="$RECOVERY_LIB_DIR/deploy-container-platform.sh"
