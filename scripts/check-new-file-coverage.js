 ...
 *
 * ClassroomPath version - checks ClassroomPath-specific workspaces.
 * OpenPath upstream is verified by OpenPath's own coverage checks.
 *
...
// Coverage report locations by workspace (ClassroomPath-specific only)
...
const EXCLUDED_PATTERNS = [
  ...
  /\.test\.(ts|tsx)$/,
  /\.spec\.(ts|tsx)$/,
  // Exclude upstream OpenPath (has its own checks)
  /^upstream\//,
];
...
function getChangedFiles() {
  try {
    ...
    return files
      ...
      .filter((f) => f.startsWith('api/src/') || f.startsWith('react-spa/src/'));
  } catch (error) {
...
function main() {
  console.log('');
  console.log('Checking coverage for new/modified ClassroomPath files...');
  ...
  if (changedFiles.length === 0) {
    console.log(
      'No ClassroomPath source files changed (or all excluded). Skipping coverage check.'
    );
    process.exit(0);
  }
