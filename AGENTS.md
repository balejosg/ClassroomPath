...
| `verify:promotion-ready`      | bash scripts/verify-production-promotion-ready.sh -- checks staging is promotion-eligible; exit 0 ready, 10 blocked (expected steady state), 1+ genuine gate error                | before initiating a production promotion                                                                                                                                |
...
| `promote:production`          | node scripts/deprecated-promotion-alias.mjs -- **deprecated no-op shim**: prints the canonical pair and exits 2 without acting                                                    | do not use; run `release:promote` (inspect) or `promote:current-staging` (execute); maintainers needing an explicit tag: `bash scripts/tag-production-release.sh <tag>` |
| `promote:production:full`     | node scripts/deprecated-promotion-alias.mjs -- **deprecated no-op shim**: prints the canonical pair and exits 2 without acting                                                    | do not use; run `npm run release:promote -- --auto-tag --execute` for the orchestrated sequence                                                                         |
...
| `release:production`          | node scripts/deprecated-promotion-alias.mjs -- **deprecated no-op shim**: prints the canonical pair and exits 2 without acting                                                    | do not use; run `promote:current-staging` (auto-derives the tag) or `bash scripts/tag-production-release.sh <tag>` (maintainers)                                        |
| `release:promote`             | node scripts/release-promote.mjs -- **dry-run by default**; with --execute runs the full promotion sequence (evidence validation, staging deploy, health check, tag push, canary) | **canonical inspect entry point** (pair with `promote:current-staging` to execute); add --execute to perform real operations                                            |
