...
function assertNightlyStagingCandidateGate(
  ...
  assert.equal(promotionReadyStep.env?.DEPLOY_USER, '${{ secrets.DEPLOY_USER }}');
...
describe('Deploy workflow contracts', () => {
  ...
  test('nightly promotion readiness verifies existing OpenPath evidence without dispatch credentials', () => {
    const workflow = readWorkflow('.github/workflows/nightly-staging-candidate.yml');
    ...
    assert.equal(
      job.steps?.some((step) => step.name === 'Create OpenPath dispatch token'),
      false
    );
  ...
  test('deploy and smoke workflows reuse shared transport, verifier, and concurrency helpers', () => {
    ...
    const rollbackProductionScript = readText('scripts/lib/production-recovery-executor.sh');
