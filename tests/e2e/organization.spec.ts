...
test.describe('Organization Members', () => {
  ...
  test('should invite new teacher to organization @org @invite', async ({ page }) => {
    const orgPage = new OrganizationPage(page);
    await orgPage.goto();
    ...
    // Should show success
    await expect(page.getByText(/invitación enviada|invitation sent|éxito/i)).toBeVisible({ timeout: 5000 });
  });
...
test.describe('Teacher Permissions', () => {
  ...
  test('should prevent teacher from inviting users @org @permissions', async ({ page }) => {
    ...
    const hasAccess = !currentUrl.includes('organization') || 
...
test.describe('Multi-Organization (Future)', () => {
  test.skip('should allow user to switch between organizations @org @multi-org', async ({ page }) => {
