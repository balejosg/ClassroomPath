...
function writeHighRiskVerificationState(path: string, overrides: string[] = []) {
  writeFileSync(
    ...
    [
      ...
      'STAGING_WINDOWS_BOOTSTRAP_CANARY_FAILURE_BOUNDARY_MESSAGE=Windows page-resource observation completed without automatic rule creation and explicit allowlist probes succeeded.',
...
test('release-state CLI verifies staging evidence and emits workflow outputs', () => {
  ...
  writeFileSync(
    ...
    [
      ...
      'STAGING_LINUX_BOOTSTRAP_FAILURE_BOUNDARY_MESSAGE=Linux page-resource observation completed without automatic rule creation and explicit allowlist probes succeeded.',
